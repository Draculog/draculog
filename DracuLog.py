...
if len(sys.argv) > 1:
    if 'test' in sys.argv or 't' in sys.argv:
        ...
        runTest = True
    elif 'greencode' in sys.argv or 'g' in sys.argv:
        print("Running and Pulling Code from GreenCode")
        runGreenCode = True
        ...
        print(
            "Commands are as follows:\nconfig/c == Use the config.ini file (ReadMe.ini) instead of taking in input at "
            "run time\n")
        print("buildConfig/bc == Build a stock config.ini file if you have deleted yours\n")
        print("buildParam/bp == Build a sample param.ini file in your source folder if you don't have one\n")
        print("greencode/g == Pull and run code from GreenCode website/Database\n")
        sys.exit()
    else:
...
import shutil  # used to copy raw cvs's elsewhere
...
import 3d  # for date processing for pyRAPL
import mysql.connector

if runTest:
    print("Attempting to Set up Connection to GreenCode")
    cnx = mysql.connector.connect(user='g0bxsk9qvnm7tcxk', password='h4dwasipdkzv987v',
                                  host='uyu7j8yohcwo35j3.cbetxkdyhwsb.us-east-1.rds.amazonaws.com', port=3306,
                                  database='sva2irh4cex0i9m9')
    cursor = cnx.cursor()
...
if runGreenCode:
    print("Setting up Connection to GreenCode")
    cnx = mysql.connector.connect(user='g0bxsk9qvnm7tcxk', password='h4dwasipdkzv987v',
                                  host='uyu7j8yohcwo35j3.cbetxkdyhwsb.us-east-1.rds.amazonaws.com', port=3306,
                                  database='sva2irh4cex0i9m9')
    print(cnx)
...
data_list = []  # to store all data in 2D Dictionary structured as such ->
# data_list = [
#			{	'run_number':"run x",
...
class Manager:
    ...
    def read_configs(self):
        ...
        global sourceDir, paramsFile, csvFile, jsonFile, toCSV, toJSON, controlTemp, baselineTemp, runCpuTempLog, cpuInterval
        global runLoadLog, loadInterval, runEnergyLog, energyInterval, useMakerHawk, usePyRAPL, runClean, showTempCycles
    ...
    def run_baseline(self):
        ...
        if runEnergyLog and useMakerHawk and not runTest and not usePyRAPL:
            print(
                "Please get your MakerHawk software ready, as there will be a 15 second count down once you hit enter below")
        ...
        if runEnergyLog:
            if usePyRAPL:
                try:
                    ...
                    print(
                        "Looks like your system has un-chmod'ed a RAPL file. Attemping to fix this issu using this command:")
    ...
    def run_loggers(self):
        ...
        if runEnergyLog and useMakerHawk and not runTest and not usePyRAPL:
            print(
                "Please get your MakerHawk software ready, as there will be a 10 second count down once you hit enter below")
            print("This is needed to align your Amp's CSV data from your Makerhawk with your other sensor data")
            print("So make sure you clear the AMPS data/sensor/log in the Makerhawk software first please!")
            confirm = input("Hit Enter, then a 15 second countdown will start.....")
            self.count_down(15)
    ...
    def cool_down(self):
        if onPI:
            ...
            while round(psutil.sensors_temperatures()['coretemp'][0][1]) > (baselineTemp - 3):
                time.sleep(5)
            self.cooldownEnd = time.time()
            self.cooldownDelta = self.cooldownEnd - self.cooldownStartprint(
                "Delta Cool Down time is " + str(self.cooldownDelta))
    ...
    def data_to_csv(self):
        ...
        if controlTemp:
            time_header_keys.append(
                ["Cooldown Delta", "Cooldown Start", "Cooldown End", "Warmup Delta", "Warmup Start", "Warmup End"])
        header_row = []
        time_row = []
    ...
    def gather_energy_data(self):
        ...
        if useMakerHawk:
            ...
            confirm = input(
                "Press enter when you've finished importing the Amps and Volts csv's into your Source/dir folder (where you code is).....")
...
class CpuLog:
    ...
    def log(self):
        while continueLogging:
            try:
                cpuTemp = psutil.sensors_temperatures()['coretemp'][0][
                    1]  # Grabs sensors, then the CPU specific temp, then PKG temp, then temp
            except RunTimeError:
                self.failure += 1
                cpuTemp = 0.0
                pass
